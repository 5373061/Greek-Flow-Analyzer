...
class IntegratedDashboard:
    ...
    def load_recommendations(self, recommendations_dir="results/ml_predictions/recommendations"):
        """Load recommendations from the results directory."""
        self.recommendations = []
        
        # Check if directory exists
        if not os.path.exists(recommendations_dir):
            self.update_status(f"Recommendations directory not found: {recommendations_dir}")
            return
        
        # Find all recommendation files
        recommendation_files = []
        for ext in ["*.json"]:
            pattern = os.path.join(recommendations_dir, ext)
            recommendation_files.extend(glob.glob(pattern))
        
        logger.info(f"Found {len(recommendation_files)} recommendation files")
        
        # Load each recommendation file
        for file_path in recommendation_files:
            try:
                with open(file_path, 'r') as f:
                    recommendation = json.load(f)
                    
                    # Format recommendation for dashboard
                    formatted_rec = self.format_recommendation_for_dashboard(recommendation)
    ...
    def create_filter_controls(self):
        ...
        self.strategy_combo['values'] = ['All', 'Momentum', 'Mean Reversion', 'Volatility Expansion', 'Greek Flow', 'ML Enhanced']
    ...
    def update_detail_view(self, recommendation):
        ...
        # Try to load ML prediction data
        ticker = recommendation.get("symbol", "")
        ml_prediction = None
        if ticker:
            ml_prediction = self.load_ml_prediction(ticker)
            
            # If ML prediction found, enhance recommendation with it
            if ml_prediction:
                # Add ML prediction data to recommendation
                recommendation["ml_prediction"] = ml_prediction
                
                # Add trade signals if available
                trade_signals = ml_prediction.get("trade_signals", {})
                if trade_signals:
                    recommendation["trade_signals"] = trade_signals
                
                # Add entropy patterns if available
                entropy_patterns = ml_prediction.get("entropy_patterns", [])
                if entropy_patterns:
                    recommendation["entropy_patterns"] = entropy_patterns
        
        # Update summary tab
        self.update_summary_tab(recommendation)
        
        # Update entry/exit tab
        self.update_entry_exit_tab(recommendation)
        
        # Update Greeks tab
        self.update_greeks_tab(recommendation)
        
        # Update market context tab
        self.update_market_context_tab(recommendation)
        
        # Update regime tab
        self.update_regime_tab(recommendation)
        
        # Update duration tab
        self.update_duration_tab(recommendation)
