...
class GreekEnergyAnalyzer:
    ...
    def format_results(
        ...
        Take the raw analysis_results and a spot_price, and return a dict of:
          ...
          - a full_report text string assembled via _generate_full_report
        """
        ...
        try:
            ...
            if vp.get('price_points'):
                formatted['vanna_projections'] = {
                    ...
                    'projections': {
                        ...
                        for k, vs in vp.get('projections', {}).items()
                    }
                }
            ...
            if cp.get('price_points'):
                formatted['charm_projections'] = {
                    ...
                    'projections': {
                        k: [f"{float(val):.6f}" for val in vs]
                        for k, vs in cp.get('projections', {}).items()
                    }
                }

            # --- Final Text Report ---
            # Placeholders for support / resistance formatting
            formatted_support_str = "Support Levels (Formatted Text Placeholder)"
            formatted_resistance_str = "Resistance Levels (Formatted Text Placeholder)"
            ...
            formatted['full_report'] = _generate_full_report(
            ...
            logger.error(
                f"Error in GreekEnergyAnalyzer.format_results: {e}",
                exc_info=True
            )
