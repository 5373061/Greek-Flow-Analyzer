...
class IntegratedDashboard:
    ...
    def __init__(self, root, base_dir=None):
        ...
        # Initialize chart_frame attribute
    ...
    def load_all_data(self, extract_from_recommendations=True):
        """
        Load recommendations and related data.
        
        Args:
            extract_from_recommendations: If True, extract additional data from recommendations
                                         If False, load from separate files
        """
        ...
        if not self.recommendations:
            logger.warning("No recommendations loaded, cannot extract additional data")
            self.update_status("No recommendations found", "warning")
            return
        
        if extract_from_recommendations:
            logger.info("Extracting additional data from recommendations...")
            # Extract market regime data from recommendations
            self.extract_market_regime_from_recommendations()
            
            # Extract energy levels and reset points from recommendations
            self.extract_energy_and_reset_from_recommendations()
            
            # Extract Greek data from recommendations
            self.extract_greek_data_from_recommendations()
            
            # Extract trade duration data from recommendations
            self.extract_duration_from_recommendations()
        else:
            logger.info("Loading additional data from files...")
        ...
        self.update_status("All data loaded successfully", "info")
    ...
    def extract_market_regime_from_recommendations(self):
        """Extract market regime data directly from recommendations."""
        self.market_regimes = {}
        self.instrument_regimes = {}
        
        logger.info("Extracting market regime data from recommendations...")
        self.update_status("Extracting market regime data...")
        ...
        for rec in self.recommendations:
            ...
            # Option 1: Direct market_regime field
            if "market_regime" in raw_data and isinstance(raw_data["market_regime"], dict):
                regime_data = raw_data["market_regime"]
                
                # Check if it's a general market regime or instrument-specific
                if "market" in regime_data.get("scope", "").lower() or "general" in regime_data.get("scope", "").lower():
                    # General market regime
                    self.market_regimes["primary"] = regime_data
                    logger.info(f"Extracted general market regime from {symbol} recommendation")
                else:
                    # Instrument-specific regime
                    self.instrument_regimes[symbol] = regime_data
                    logger.info(f"Extracted instrument-specific regime for {symbol}")
                continue
            
            # Option 2: In analysis_results
            if "analysis_results" in raw_data and isinstance(raw_data["analysis_results"], dict):
                analysis = raw_data["analysis_results"]
                
                if "market_regime" in analysis and isinstance(analysis["market_regime"], dict):
                    regime_data = analysis["market_regime"]
                    
                    # Check if it's a general market regime or instrument-specific
                    if "market" in regime_data.get("scope", "").lower() or "general" in regime_data.get("scope", "").lower():
                        # General market regime
                        self.market_regimes["primary"] = regime_data
                        logger.info(f"Extracted general market regime from {symbol} analysis results")
                    else:
                        # Instrument-specific regime
                        self.instrument_regimes[symbol] = regime_data
                        logger.info(f"Extracted instrument-specific regime for {symbol} from analysis results")
            
            # Option 3: In ml_predictions
            if "ml_predictions" in raw_data and isinstance(raw_data["ml_predictions"], dict):
                ml_pred = raw_data["ml_predictions"]
                
                if "primary_regime" in ml_pred and isinstance(ml_pred["primary_regime"], dict):
                    # Create a simplified regime structure
                    regime_data = {
                        "primary_label": ml_pred["primary_regime"].get("prediction", "Unknown"),
                        "confidence": ml_pred["primary_regime"].get("confidence", 0.5),
                        "source": "ml_predictions"
                    }
                    
                    # Add volatility regime if available
                    if "volatility_regime" in ml_pred and isinstance(ml_pred["volatility_regime"], dict):
                        regime_data["volatility_label"] = ml_pred["volatility_regime"].get("prediction", "Normal")
                        regime_data["volatility_confidence"] = ml_pred["volatility_regime"].get("confidence", 0.5)
                    
                    # Assume it's a general market regime
                    self.market_regimes["primary"] = regime_data
                    logger.info(f"Extracted market regime from {symbol} ML predictions")
        
        # Log results
        logger.info(f"Extracted {len(self.market_regimes)} general market regimes and {len(self.instrument_regimes)} instrument-specific regimes")
        self.update_status(f"Extracted market regime data")
    ...
    def extract_energy_and_reset_from_recommendations(self):
        ...
        for rec in self.recommendations:
            ...
            # Check for energy levels directly in recommendation
            if "energy_levels" in rec and isinstance(rec["energy_levels"], list):
                self.energy_levels[symbol] = rec["energy_levels"]
                logger.info(f"Extracted energy levels for {symbol} from recommendation")
            
            # Check for reset points directly in recommendation
            if "reset_points" in rec and isinstance(rec["reset_points"], list):
                self.reset_points[symbol] = rec["reset_points"]
                logger.info(f"Extracted reset points for {symbol} from recommendation")
            
            # If not found directly, check in raw_data
            raw_data = rec.get("raw_data", {})
            
            if symbol not in self.energy_levels and "energy_levels" in raw_data:
                self.energy_levels[symbol] = raw_data["energy_levels"]
                logger.info(f"Extracted energy levels for {symbol} from raw_data")
            
            if symbol not in self.reset_points and "reset_points" in raw_data:
                self.reset_points[symbol] = raw_data["reset_points"]
                logger.info(f"Extracted reset points for {symbol} from raw_data")
            
            # Check in analysis_results if present
            if "analysis_results" in raw_data:
                analysis = raw_data["analysis_results"]
                
                if symbol not in self.energy_levels and "energy_levels" in analysis:
                    self.energy_levels[symbol] = analysis["energy_levels"]
                    logger.info(f"Extracted energy levels for {symbol} from analysis_results")
                    
                if symbol not in self.reset_points and "reset_points" in analysis:
                    self.reset_points[symbol] = analysis["reset_points"]
                    logger.info(f"Extracted reset points for {symbol} from analysis_results")
    ...
    def extract_greek_data_from_recommendations(self):
        ...
        for rec in self.recommendations:
            ...
            # Check for Greek data in various possible locations
            raw_data = rec.get("raw_data", {})
            
            # Option 1: Direct greek_data field
            if "greek_data" in rec and isinstance(rec["greek_data"], dict):
                self.greek_data[symbol] = rec["greek_data"]
                logger.info(f"Extracted Greek data for {symbol} from recommendation")
                continue
            
            # Option 2: Greek metrics directly in recommendation
            greek_fields = ["delta", "gamma", "theta", "vega", "rho", "vanna", "charm"]
            if any(field in rec for field in greek_fields):
                self.greek_data[symbol] = {field: rec[field] for field in greek_fields if field in rec}
                logger.info(f"Extracted Greek metrics for {symbol} from recommendation fields")
                continue
            
            # Option 3: In raw_data
            if "greek_analysis" in raw_data:
                self.greek_data[symbol] = raw_data["greek_analysis"]
                logger.info(f"Extracted Greek data for {symbol} from raw_data")
                continue
            
            # Option 4: In analysis_results
            if "analysis_results" in raw_data and "greek_analysis" in raw_data["analysis_results"]:
                self.greek_data[symbol] = raw_data["analysis_results"]["greek_analysis"]
                logger.info(f"Extracted Greek data for {symbol} from analysis_results")
                continue
    ...
    def extract_duration_from_recommendations(self):
        ...
        for rec in self.recommendations:
            ...
            # Skip if we already have data for this strategy
            if strategy in self.trade_durations:
                continue
            
            # Check for duration data in various possible locations
            raw_data = rec.get("raw_data", {})
            
            # Option 1: Direct duration field
            if "duration_profile" in rec:
                self.trade_durations[strategy] = {"completion_profile": rec["duration_profile"]}
                logger.info(f"Extracted duration profile for {strategy} from recommendation")
                continue
            
            # Option 2: In raw_data
            if "duration_profile" in raw_data:
                self.trade_durations[strategy] = {"completion_profile": raw_data["duration_profile"]}
                logger.info(f"Extracted duration profile for {strategy} from raw_data")
                continue
            
            # Option 3: In strategy_metrics
            if "strategy_metrics" in raw_data and "duration_profile" in raw_data["strategy_metrics"]:
                self.trade_durations[strategy] = {"completion_profile": raw_data["strategy_metrics"]["duration_profile"]}
                logger.info(f"Extracted duration profile for {strategy} from strategy_metrics")
                continue
    ...
    def update_price_chart(self, rec):
        ...
        # Safety check for chart_frame
        if not hasattr(self, 'chart_frame') or self.chart_frame is None:
            self.chart_frame = tk.Frame(self.root)
            return
"""Update the price chart with available data, gracefully handling missing data."""
        ...
        # Check if we have price levels to plot
        has_energy_levels = symbol in self.energy_levels and self.energy_levels[symbol]
        has_reset_points = symbol in self.reset_points and self.reset_points[symbol]
        
        # Also check if they're directly in the recommendation
        if not has_energy_levels and 'energy_levels' in rec and rec['energy_levels']:
            has_energy_levels = True
            # Store for future use
            self.energy_levels[symbol] = rec['energy_levels']
        
        if not has_reset_points and 'reset_points' in rec and rec['reset_points']:
            has_reset_points = True
            # Store for future use
            self.reset_points[symbol] = rec['reset_points']
        
        if not has_energy_levels and not has_reset_points:
            # No price levels to plot, show informative message
            ttk.Label(
                self.chart_frame, 
                text=f"No price level data available for {symbol}.\nThis is normal for 'Wait for Better Setup' recommendations.",
                justify="center"
            ).pack(padx=20, pady=30)
            return
        
        # Continue with chart creation... (rest of the function remains the same)
        try:
            symbol = rec.get('symbol', 'Unknown')
            current_price = rec.get('current_price', 0)
            entry_zone = rec.get('entry_zone', [0, 0])
    ...
    def load_greek_data(self):
        ...
        if self.recommendations:
            symbols_to_check = set(rec["symbol"] for rec in self.recommendations if rec and "symbol" in rec)
            logger.info(f"Checking Greek data for {len(symbols_to_check)} symbols from recommendations")
        else:
            logger.warning("No recommendations loaded, cannot check greek files by symbol")
            symbols_to_check = set()
        ...
        # First, check for Greek data in analysis files
        for symbol in symbols_to_check:
            analysis_path = os.path.join(self.results_dir, f"{symbol}_analysis.json")
            ...
            if os.path.exists(analysis_path):
                try:
                    ...
                    if isinstance(analysis_data, dict):
                        # Extract greek_analysis section
                        greek_analysis = analysis_data.get("greek_analysis", analysis_data)
                    ...
                    logger.error(f"Error loading Greek data for {symbol} from {analysis_path}: {e}")
            else:
                logger.debug(f"Analysis file not found for {symbol}: {analysis_path}")
        
        # Log summary of what was found and what's missing
        if loaded_greek_count > 0:
            logger.info(f"Loaded Greek data for {loaded_greek_count} symbols")
            missing_symbols = symbols_to_check - set(self.greek_data.keys())
            if missing_symbols:
                logger.info(f"Missing Greek data for {len(missing_symbols)} symbols: {', '.join(missing_symbols)}")
        else:
            logger.warning(f"No Greek data found for any symbols. Expected files like SYMBOL_analysis.json with greek_analysis section")
        
        self.update_status(f"Loaded Greek data for {loaded_greek_count}/{len(symbols_to_check)} symbols")

    def load_all_data(self):
        """Load recommendations and extract all available data."""
        logger.info("Starting data load...")
        self.update_status("Loading all data...")
        ...
        # Extract data from recommendations where possible
        if self.recommendations:
            # Extract market regime data from recommendations
        ...
        # Fall back to loading from separate files if needed
        if not self.market_regimes:
            self.load_market_regime_data()
            
        if not any(self.energy_levels.values()) and not any(self.reset_points.values()):
            self.load_energy_and_reset_data()
            
        if not self.greek_data:
